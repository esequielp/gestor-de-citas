...
const LandingPage: React.FC<Props> = ({ onNavigate }) => {
  ...
  useEffect(() => {
    // Load top active services and branches from the shared data service
    setServices(dataService.getServices().filter(s => s.active).slice(0, 6)); 
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
              ...
              <div>
                <h1 className="text-xl font-bold tracking-tight uppercase">Estilo<span className="text-indigo-600">Pro</span></h1>
                <p className="text-[10px] text-gray-500 tracking-widest uppercase -mt-1">Beauty & Spa</p>
              </div>
            </div>
            ...
            {services.map((service, index) => (
              <div key={service.id} className="bg-white p-8 rounded-2xl shadow-sm hover:shadow-xl transition-all border border-gray-100 group">
                <div className="flex justify-between items-start mb-6">
                   ...
                   <span className="font-serif text-2xl font-bold text-indigo-600">${service.price}</span>
                </div>
                <h4 className="text-xl font-bold text-gray-900 mb-3 group-hover:text-indigo-600 transition-colors">{service.name}</h4>
                <p className="text-gray-500 mb-6 leading-relaxed min-h-[3rem]">{service.description}</p>
      ...
      <section id="contacto" className="py-24 bg-indigo-50">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div className="bg-white rounded-2xl shadow-xl p-8 md:p-12">
                ...
                <form className="space-y-4 text-left" onSubmit={(e) => { e.preventDefault(); alert('Mensaje enviado (simulado)'); }}>
                    <div className="grid md:grid-cols-2 gap-4">
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1">NOMBRE</label>
                            <input type="text" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="Tu nombre"/>
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-gray-500 mb-1">EMAIL</label>
                            <input type="email" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none" placeholder="tucorreo@ejemplo.com"/>
                        </div>
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-gray-500 mb-1">MENSAJE</label>
                        <textarea className="w-full p-3 bg-gray-50 border border-gray-200 rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none h-32" placeholder="¿En qué podemos ayudarte?"></textarea>
                    </div>
                    <Button fullWidth size="lg">Enviar Mensaje</Button>
                </form>
            </div>
        </div>
      </section>
      ...
      <footer className="bg-gray-900 text-gray-300 py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid md:grid-cols-4 gap-12 border-b border-gray-800 pb-12">
                <div className="col-span-1 md:col-span-2">
                    <div className="flex items-center gap-2 mb-6">
                        ...
                        <h3 className="text-xl font-bold text-white uppercase">EstiloPro</h3>
                    </div>
            ...
            <div className="pt-8 flex flex-col md:flex-row justify-between items-center gap-4 text-xs text-gray-600">
                <p>© 2024 EstiloPro. Todos los derechos reservados.</p>
